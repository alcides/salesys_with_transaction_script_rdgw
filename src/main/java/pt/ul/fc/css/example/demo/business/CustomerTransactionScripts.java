...
/**
 * Handles customer transactions. Each public method implements a transaction script.
 *
 * @author fmartins
...
public class CustomerTransactionScripts {
  ...
  /**
   * Adds a new customer. It checks that there is no other customer in the system with the same VAT.
   *
   ...
   * @throws ApplicationException When the VAT number is already in the database, when the VAT
   *     number is invalid (we check according to the Portuguese legislation), or when there is some
  ...
  public void addCustomer(int vat, String denomination, int phoneNumber, DiscountType discountType)
    ...
    if (!isValidVAT(vat)) throw new ApplicationException("Invalid VAT number: " + vat);

    // Checks that denomination and phoneNumber and filled in
    if (!isFilled(denomination) || phoneNumber == 0)
      throw new ApplicationException("Both denomination and phoneNumber must be filled");
    ...
    // insertions of customers with the same VAT number, I simple assume that the
    // number
    // is not in the database. In situations where a high number of collisions is
    // expected,
    // the best way is to follow a pessimistic approach, which relies in locking
    // the table, checking insertion success, and unlocking the table at the end of
    // the
    // operation. This topic will be discussed in greater detail in the context of
    // object
    // relational mapping (ORM).
    // Anyway, be careful that testing for the existence and then inserting
    // (without locking or other suitable method for handling concurrency)
    // introduces a
    // race condition in the application.
    try {
      CustomerRowDataGateway newCustomer =
          new CustomerRowDataGateway(vat, denomination, phoneNumber, discountType);
      newCustomer.insert();
    } catch (PersistenceException e) {
      // Another important this to notice is exception wrapping. Notice that the
      // persistence
      // exception thrown by the underlying JDBC API is not discarded; instead, it is
      // wrapped
      // under an application exception. This way we still know the precise problem
      // that occurred
      // but prevent from showing low-level exceptions to the end users. It is not
      // pleasant at
  ...
  private boolean isValidVAT(int vat) {
    ...
    if (vat < 100000000 || vat > 999999999) return false;
    ...
    if (checkDigitCalc == 10) checkDigitCalc = 0;
