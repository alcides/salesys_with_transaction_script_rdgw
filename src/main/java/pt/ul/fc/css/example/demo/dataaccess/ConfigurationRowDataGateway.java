 ...
 *
 * <p>Here we illustrate another form of handling singleton objects. The object is created only when
 * information is needed (lazy creation) and the information is cached in final attributes,
 * preventing future accesses to the database. In this example we are assuming that there are no
 * changes to the application parameters during its execution. Should we need to handled this
 * situation, then we would have to access the database in every read. See the discussion about
 * thread safety in the DataSource class.
 *
 * @author fmartins
...
public class ConfigurationRowDataGateway {
  ...
  private ConfigurationRowDataGateway(
      double totalAmountPercentage, double amountThreshold, double eligiblePercentage) {
  ...
  /**
   * @return The sale's amount threshold for being eligible for a total sale discount.
   */
  ...
  private static final String GET_APP_CONFIG_SQL =
      "select id, totalAmountPercentage, amountThreshold, eligiblePercentage " + "from appconfig ";
  ...
  public static ConfigurationRowDataGateway getConfiguration() throws PersistenceException {
    if (configurationRow == null)
      try (DataSourceWrapper dsw = new DataSourceWrapper()) {
        try (PreparedStatement comando = dsw.prepare(GET_APP_CONFIG_SQL)) {
          try (ResultSet rs = comando.executeQuery()) {
            rs.next();
            // creates the singleton configuration row
            configurationRow =
                new ConfigurationRowDataGateway(
                    rs.getDouble("totalAmountPercentage"),
                    rs.getDouble("amountThreshold"),
                    rs.getDouble("eligiblePercentage"));
          }
