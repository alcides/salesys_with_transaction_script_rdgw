...
public class ProductRowDataGateway {
  ...
  /**
   * The product code. This code is the one present in the product that customers have access to. It
   * is different from the id, which is an internal identity, used for relations with other entities
   * (e.g., Sale)
   */
  private int prodCod;
  ...
  /**
   * Comment: the product id is an internal (persistence) concept only used to link entities and
   * must not have any business meaning. Also, there is no setProductId, since the id does not
   * change through the execution of the program.
   *
  ...
  /**
   * Comment: there is a business rule to not allow product code changes. That is why there is no
   * method for updating the product code.
   *
  ...
  /**
   * Notice that the setter and the getter convert the string to a boolean value that is then used
   * in the application's domain logic.
   *
  ...
  public static ProductRowDataGateway findWithProdCod(int prodCod) throws PersistenceException {
    try (DataSourceWrapper dsw = new DataSourceWrapper()) {
      try (PreparedStatement statement = dsw.prepare(GET_PRODUCT_BY_PROD_COD_SQL)) {
  ...
  public ProductRowDataGateway find(int id) throws PersistenceException {
    try (DataSourceWrapper dsw = new DataSourceWrapper()) {
      try (PreparedStatement statement = dsw.prepare(GET_PRODUCT_BY_ID_SQL)) {
        // set statement arguments
  ...
  public void updateStockValue() throws PersistenceException {
    try (DataSourceWrapper dsw = new DataSourceWrapper()) {
      try (PreparedStatement statement = dsw.prepare(UPDATE_STOCK_SQL)) {
        // set statement arguments
        ...
        throw new PersistenceException(
            "Internal error updating product " + id + " stock amount.", e);
      }
    }
  }
