...
import pt.ul.fc.css.example.demo.dataaccess.ConfigurationRowDataGateway;
import pt.ul.fc.css.example.demo.dataaccess.CustomerRowDataGateway;
...
import pt.ul.fc.css.example.demo.dataaccess.ProductRowDataGateway;
import pt.ul.fc.css.example.demo.dataaccess.RecordNotFoundException;
import pt.ul.fc.css.example.demo.dataaccess.SaleProductRowDataGateway;
import pt.ul.fc.css.example.demo.dataaccess.SaleRowDataGateway;

/**
 * Handles sales' transactions. Each public method implements a transaction script.
 *
 * <p>Notes: 1. Check the transaction control in the addProductToSale method. I need to make sure
 * that both tables were updated consistently: products table (the product stock) and the sales
 * table (insert the sales record). If some error occurs, the transaction is rolled back and an
 * exception is thrown wrapping the underlying error for debugging purposes. See the catch 6 lines
 * below the throw that wraps the specific exception in a more general message that can be shown to
 * the end user. This way, the end user is spared of low-level details that only make sense to a
 * specialized user.
 *
 * @author fmartins
...
public class SaleTransactionScripts {
   ...
   * @return The id of the new sale. This id is useful for adding products to the sale and for
   *     computing the sale's total discount, etc.
   * @throws ApplicationException When the customer does not exist or when an unexpected SQL error
   *     occurs.
   */
   ...
   * @throws ApplicationException When the sale is closed, the product code does not exist, there's
   *     not enough amount of product to be sold, or when same unexpected SQL error occurs.
   * @pre: qty > 0
   */
  public void addProductToSale(int saleId, int productCode, double qty)
    ...
    try {
      ...
      // glance we may argue that this is impossible due to the following if
      // statement,
      ...
      if (product.getQty() >= qty) {
        ...
        throw new ApplicationException(
            "Product with code "
                ...
                + " has ("
                + product.getQty()
                + ") units, which is insufficient for the request of "
                + qty
                + " units.");
    } catch (PersistenceException e) {
   ...
   * @throws PersistenceException When there is an error inserting the sale product into the
   *     database
   */
  private void addProductToSale(ProductRowDataGateway product, SaleRowDataGateway sale, double qty)
      throws PersistenceException {
    // adds product to sale
    SaleProductRowDataGateway saleProduct =
        new SaleProductRowDataGateway(sale.getId(), product.getProductId(), qty);
    saleProduct.insert();
  }

  /**
   * Gets a SaleRowGateway object for a given sales id, or raises a RecordNotFoundException
   * otherwise.
   *
  ...
  /**
   * Gets a ProductRowGateway object for a given productCode, or raises a RecordNotFoundException
   * otherwise.
   *
   ...
   * @throws ApplicationException When the sale does not exist or when an unexpected referential
   *     integrity problem occurs.
   */
  public double getSaleDiscount(int saleId) throws ApplicationException {
    try {
      ...
      if (sale.getStatus() == SaleStatus.CLOSED) return sale.getDiscount();

      // Get customer associated with the sale.
      // The customer always exists due to the referential integrity enforced by the
      // database
      CustomerRowDataGateway customer = CustomerRowDataGateway.find(sale.getCustomerId());

      // Get sale's products
      Iterable<SaleProductRowDataGateway> saleProducts =
          SaleProductRowDataGateway.findSaleProducts(saleId);

      // Compute the sale discount
      return computeDiscount(customer, saleProducts);
    } catch (PersistenceException e) {
      throw new ApplicationException(
          "Internal referential integrity error when computing "
              + "the discount for sale "
              + saleId,
          e);
    }
  }
  ...
  private double computeDiscount(
      CustomerRowDataGateway customer, Iterable<SaleProductRowDataGateway> saleProducts)
      throws PersistenceException {
  ...
  private double discountOnSaleAmount(Iterable<SaleProductRowDataGateway> saleProducts)
    ...
    return saleTotal > config.getAmountThreshold()
        ? saleTotal * config.getAmountThresholdPercentage()
        : 0;
  }
  ...
  /**
   * Computes the sale's total of eligible products, or RecordNotFoundException in case there is a
   * referential integrity problem (the sales refers to a product that does not exits).
   *
   * <p>Notice the Predicate<...> for receiving the lambda expression and the if statement applies
   * the filter passed as a parameter. Available Since Java 8.
   *
  ...
  private double computeSaleTotal(
      Iterable<SaleProductRowDataGateway> saleProducts, Predicate<ProductRowDataGateway> filter)
      throws PersistenceException {
    double saleEligibleTotal = 0;
    for (SaleProductRowDataGateway saleProduct : saleProducts) {
      // Get product info. Always exists, unless there is an referential integrity
      // problem.
      ProductRowDataGateway product = new ProductRowDataGateway().find(saleProduct.getProductId());
      if (filter.test(product)) saleEligibleTotal += saleProduct.getQty() * product.getFaceValue();
    }
